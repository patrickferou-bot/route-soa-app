  ...
  for (const t of result.tournees) {
    for (const l of t.livraisons || []) {
      for (const field of ['lat', 'lon']) {
        ...
        const val = parseFloat(String(l[field]).replace(',', '.'));
...
- "lat" / "lon" : PRIORITÉ ABSOLUE — le bon de chargement contient les coordonnées GPS EXACTES du point de livraison (silo, pas adresse de facturation). Ces coordonnées sont affichées à droite de chaque client sous la forme "Coordonnées : Nord : 44,9235199 / Ouest : -0,7069264" (format français avec VIRGULE comme séparateur décimal, et labels "Nord"/"Ouest" au lieu de "lat"/"lon"). Tu DOIS : (1) lire le nombre après "Nord :" → c'est la latitude, (2) lire le nombre après "Ouest :" → c'est la longitude, (3) remplacer la VIRGULE par un POINT pour obtenir un nombre décimal standard. Exemple : "Nord : 44,9235199" → lat : 44.9235199 / "Ouest : -0,7069264" → lon : -0.7069264. Ces coordonnées pointent vers le silo réel sur l'exploitation — elles sont plus fiables que tout géocodage d'adresse. Ne les invente JAMAIS, ne les estime JAMAIS : si elles sont présentes dans le PDF, extrais-les exactement. Si absentes, mets null.
